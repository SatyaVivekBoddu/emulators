...
int main(int argc, char** argv) {
    ...
    SDL_Window* window = SDL_CreateWindow("Game Boy Emulator", gb::kScreenWidth * kScale,
                                          gb::kScreenHeight * kScale, 0);
            ...
            while (SDL_PollEvent(&event)) {
                ...
                if (event.type == SDL_EVENT_KEY_DOWN || event.type == SDL_EVENT_KEY_UP) {
                    ...
                    if (group) {
                        if (pressed)
                            *group &=
                                static_cast<gb::u8>(~bit); // pressed = bit cleared (active-low)
                        else
