...
u8 Ppu::statInterruptBitsAsByte() const {
    return static_cast<u8>((statInterruptOnMode0 ? 0x08 : 0) | (statInterruptOnMode1 ? 0x10 : 0) |
...
void Ppu::renderSprites() {
    if (!(lcdc & 0x02))
        return; // bit 1: sprite rendering enabled at all
    ...
    for (int i = 0; i < 40; ++i) {
        ...
        if (rowInSprite < 0 || rowInSprite >= 8)
            continue; // this sprite isn't on this scanline
        ...
        for (int px = 0; px < 8; ++px) {
            ...
            if (screenX < 0 || screenX >= kScreenWidth)
                continue;
